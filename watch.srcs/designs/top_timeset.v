module top_timeset(
	...
	wire [9:0] MS;
    wire [4:0] push;
    debounce db0(.clk(MS[2]),.in(push_switch[0]),.out(push[0]));
    debounce db1(.clk(MS[2]),.in(push_switch[1]),.out(push[1]));
    debounce db2(.clk(MS[2]),.in(push_switch[2]),.out(push[2]));
    debounce db3(.clk(MS[2]),.in(push_switch[3]),.out(push[3]));
    debounce db4(.clk(MS[2]),.in(push_switch[4]),.out(push[4]));
    ...
    rtc rtctmp(.clk(clk),.rstb(!RST_TS),.scale(6'o1_0),.ms(MS),.ms_acc(MS_REF));
    ...
    reg [7:0] EN;
    time_transform tt_normal(.rstb(!TIMESET_DONE),.mode(0),.ms_acc(MS_REF),.prst({18'o00_00_00,TIME_TARGET}),.date(),.t(T_REF));
   ...
   show_digit vis_digit (.clk(MS[0]), .rstb(rstb), .numbers(dip_switch[TIMESET_MODE_SEL]? n: n2), .digit(digit), .seg_data(seg_data),.en(dip_switch[TIMESET_MODE_SEL]? EN:EN2));
